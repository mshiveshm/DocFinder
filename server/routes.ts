...
//       }),
//       resave: false,
//       secret: process.env.SESSION_SECRET || "docfinder-secret",
...
//       try {
//         console.log("LocalStrategy: Looking up user with username:", username);
        
//         // Note: We can't easily list all users from the database storage
//         // This debug code is only relevant for MemStorage
...
//         const isMatch = password === user.password; // For simplicity in this prototype
...
//       const hashedPassword = password; // For simplicity in this prototype
...
//       const hashedPassword = newPassword; // For simplicity in this prototype
...
//   app.post("/api/documents/lost", async (req: Request, res: Response) => {
//     if (!req.isAuthenticated()) {
//       return res.status(401).json({ message: "Not authenticated" });
//     }
...
//       // Process text using Gemini AI if description is provided
//       if (req.body.description && req.body.description.trim().length > 0) {
//         try {
//           // Import the Gemini AI service
//           const { analyzeDocumentText } = await import('./services/gemini');
//           aiAnalysisResult = await analyzeDocumentText(req.body.description);
          
//           // Update document with AI analysis results
//           if (aiAnalysisResult) {
//             // Update document type if AI detected it and user didn't specify
//             const updateData: any = {
//               aiAnalysis: aiAnalysisResult
//             };
            
//             // If AI detected a document type and user selected "Other" or "Unknown"
//             if (aiAnalysisResult.documentType && 
//                 ['Other', 'Unknown'].includes(lostDocument.documentType)) {
//               updateData.documentType = aiAnalysisResult.documentType;
//             }
            
//             // If AI detected a name and the user didn't provide one
//             if (aiAnalysisResult.nameOnDocument && 
//                 (!lostDocument.nameOnDocument || lostDocument.nameOnDocument.trim() === '')) {
...
//           // Continue without AI analysis - non-critical failure
//         }
//       }
...
//         try {
//           // Save the image URL
//           await storage.updateLostDocument(lostDocument.id, {
//             hasImage: true,
//             imageUrl: `https://example.com/uploads/lost-document-${lostDocument.id}.jpg`,
//           });
          
//           // Analyze image with Gemini Vision if not already analyzed via text
//           if (!aiAnalysisResult && req.body.imageBase64.startsWith('data:image')) {
//             try {
//               // Import the Gemini AI service
//               const { analyzeDocumentImage } = await import('./services/gemini');
//               const imageAnalysisResult = await analyzeDocumentImage(req.body.imageBase64);
              
//               // Update document with AI analysis results from image
...
//                 // If AI detected a document type and user selected "Other" or "Unknown"
//                 if (imageAnalysisResult.documentType && 
//                     ['Other', 'Unknown'].includes(lostDocument.documentType)) {
//                   updateData.documentType = imageAnalysisResult.documentType;
//                 }
                
//                 // If AI detected a name and the user didn't provide one
//                 if (imageAnalysisResult.nameOnDocument && 
//                     (!lostDocument.nameOnDocument || lostDocument.nameOnDocument.trim() === '')) {
//                   updateData.nameOnDocument = imageAnalysisResult.nameOnDocument;
//                 }
                
//                 await storage.updateLostDocument(lostDocument.id, updateData);
//               }
//             } catch (aiImageError) {
//               console.error("AI image analysis error:", aiImageError);
//               // Continue without AI image analysis - non-critical failure
//             }
//           }
//         } catch (imageError) {
//           console.error("Image processing error:", imageError);
//           // Continue without image processing - non-critical failure
//         }
//       }
      
//       // Get the most up-to-date document after all updates
//       const updatedLostDocument = await storage.getLostDocument(lostDocument.id) || lostDocument;
      
//       // Find potential matches using AI
//       try {
//         const foundDocuments = await storage.getFoundDocuments();
        
//         // If we have found documents to match against
//         if (foundDocuments.length > 0) {
//           // Use Gemini AI for sophisticated matching
//           const { matchDocuments } = await import('./services/gemini');
//           const aiMatches = await matchDocuments(updatedLostDocument, foundDocuments);
          
//           // Create matches in the system based on AI confidence
//           for (const matchResult of aiMatches) {
//             if (matchResult.confidence >= 50) { // Only create matches with reasonable confidence
//               const matchData = {
//                 lostDocumentId: updatedLostDocument.id,
//                 foundDocumentId: matchResult.foundDocumentId,
...
//                 userId: user.id,
//                 message: `We found a potential match for your lost ${updatedLostDocument.documentType} document (${matchResult.confidence}% confidence)`,
//                 type: 'match',
//                 relatedId: match.id,
//               });
//             }
//           }
//         } else {
//           // Fallback to basic matching if no AI results or no found documents
//           const matches = foundDocuments.filter(doc => doc.documentType === updatedLostDocument.documentType);
          
//           // Create matches in the system
//           for (const matchedDoc of matches) {
//             const matchData = {
//               lostDocumentId: updatedLostDocument.id,
//               foundDocumentId: matchedDoc.id,
//               matchConfidence: 70, // Default confidence for type-based matching
//             };
...
//             await storage.createNotification({
//               userId: user.id,
//               message: `We found a potential match for your lost ${updatedLostDocument.documentType} document`,
//               type: 'match',
//               relatedId: match.id,
//             });
//           }
//         }
//       } catch (matchingError) {
//         console.error("Document matching error:", matchingError);
//         // Continue without matching - non-critical failure
//       }
      
//       // Return the updated document
//       return res.status(201).json(updatedLostDocument || lostDocument);
//     } catch (error) {
//       console.error("Report lost document error:", error);
//       return res.status(500).json({ message: "Error reporting lost document" });
//     }
//   });

//   // Report found document
//   app.post("/api/documents/found", async (req: Request, res: Response) => {
//     if (!req.isAuthenticated()) {
//       return res.status(401).json({ message: "Not authenticated" });
//     }
...
//             // If AI detected a document type and user selected "Other" or "Unknown"
//             if (aiAnalysisResult.documentType && 
//                 ['Other', 'Unknown'].includes(foundDocument.documentType)) {
//               updateData.documentType = aiAnalysisResult.documentType;
//             }
...
//         try {
//           // Save the image URL
//           await storage.updateFoundDocument(foundDocument.id, {
//             hasImage: true,
//             imageUrl: `https://example.com/uploads/found-document-${foundDocument.id}.jpg`,
//           });
...
//       // Get the most up-to-date document after all updates
//       const updatedFoundDocument = await storage.getFoundDocument(foundDocument.id) || foundDocument;
      
//       // Find potential matches using AI
//       try {
//         const lostDocuments = await storage.getLostDocuments();
        
//         // If we have lost documents to match against
//         if (lostDocuments.length > 0) {
//           // Use Gemini AI for sophisticated matching
//           // We pass our found document as the "lostDoc" parameter but swap the results
//           const { matchDocuments } = await import('./services/gemini');
//           const aiMatches = await matchDocuments(updatedFoundDocument, lostDocuments);
          
//           // Create matches in the system based on AI confidence
//           for (const matchResult of aiMatches) {
//             if (matchResult.confidence >= 50) { // Only create matches with reasonable confidence
//               const matchData = {
//                 lostDocumentId: matchResult.foundDocumentId, // This is actually the lost document ID
//                 foundDocumentId: updatedFoundDocument.id,
//                 matchConfidence: matchResult.confidence,
//               };
              
//               const match = await storage.createMatch(matchData);
              
//               // Create notification for the owner of the lost document
//               await storage.createNotification({
//                 userId: (lostDocuments.find(d => d.id === matchResult.foundDocumentId))?.userId || user.id,
//                 message: `Someone found a ${updatedFoundDocument.documentType} that might be your lost document (${matchResult.confidence}% confidence)`,
//                 type: 'found',
//                 relatedId: match.id,
//               });
//             }
//           }
//         } else {
//           // Fallback to basic matching if no AI results
//           const matches = lostDocuments.filter(doc => doc.documentType === updatedFoundDocument.documentType);
          
//           // Create matches in the system
//           for (const matchedDoc of matches) {
//             const matchData = {
//               lostDocumentId: matchedDoc.id,
//               foundDocumentId: updatedFoundDocument.id,
//               matchConfidence: 70, // Default confidence for type-based matching
//             };
...
//             // Create notification for the owner of the lost document
//             await storage.createNotification({
//               userId: matchedDoc.userId,
//               message: `Someone found a document that might be your ${matchedDoc.documentType}`,
//               type: 'found',
//               relatedId: match.id,
//             });
//           }
...
//       // Return the updated document
//       return res.status(201).json(updatedFoundDocument || foundDocument);
//     } catch (error) {
//       console.error("Report found document error:", error);
//       return res.status(500).json({ message: "Error reporting found document" });
//     }
//   });

//   // Get user's lost documents
//   app.get("/api/documents/lost", async (req: Request, res: Response) => {
//     if (!req.isAuthenticated()) {
//       return res.status(401).json({ message: "Not authenticated" });
//     }
...
//     try {
//       const user = req.user as any;
//       const lostDocuments = await storage.getUserLostDocuments(user.id);
...
//   app.get("/api/documents/found", async (req: Request, res: Response) => {
//     if (!req.isAuthenticated()) {
//       return res.status(401).json({ message: "Not authenticated" });
//     }
...
//     try {
//       const user = req.user as any;
//       const foundDocuments = await storage.getUserFoundDocuments(user.id);
...
//       let results;
//       if (type === 'lost') {
//         results = await storage.searchLostDocuments({
//           query: query as string || '',
//           documentType: documentType as string || '',
//           location: location as string || '',
//         });
//       } else {
//         results = await storage.searchFoundDocuments({
//           query: query as string || '',
//           documentType: documentType as string || '',
//           location: location as string || '',
//         });
...
//         ...n,
//         isRead: n.read // Map the field name
//       }));
...
//       // Get user's documents and matches
//       const lostDocuments = await storage.getUserLostDocuments(user.id);
//       const foundDocuments = await storage.getUserFoundDocuments(user.id);
//       const matches = await storage.getUserMatches(user.id);
//       const pendingMatches = matches.filter(match => match.status === 'pending');
//       const successfulMatches = matches.filter(match => match.status === 'matched');
      
//       // Get recent activity (combine lost, found, and match events)
//       const activities = [...lostDocuments, ...foundDocuments, ...matches]
//         .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())
//         .slice(0, 5)
//         .map(item => {
//           if ('nameOnDocument' in item) {
//             // It's a lost document
//             return {
//               icon: 'file-upload',
//               title: `You reported ${item.documentType} as lost`,
//               time: new Date(item.createdAt).toLocaleString(),
//               status: item.status,
//             };
//           } else if ('dateFound' in item) {
//             // It's a found document
//             return {
//               icon: 'hand-holding',
//               title: `You reported finding a ${item.documentType}`,
//               time: new Date(item.createdAt).toLocaleString(),
//               status: item.status,
//             };
//           } else {
...
//             return {
//               icon: 'check',
//               title: 'Match found for your document',
//               time: new Date(item.createdAt).toLocaleString(),
//               status: item.status,
//             };
//           }
...
//       const dashboardData = {
//         potentialMatches: pendingMatches.length,
//         activities,
//         stats: {
//           documentsLost: lostDocuments.length,
//           documentsFound: foundDocuments.length,
